...
export const dict: Dict = {
  ...
  // Subsidy / payback calculator
  calcTitle: { uz: 'Tomchilatib sug‘orish — foyda hisobi', ru: 'Капельный полив — расчёт выгоды' },
  calcSub: { uz: 'Tomchilatib sug‘orishga o‘tsangiz qancha tejaysiz va u qachon o‘zini oqlaydi', ru: 'Сколько сэкономите на капельном поливе и когда он окупится' },
  calcSystemCost: { uz: 'Tizim narxi (1 ga)', ru: 'Стоимость системы (1 га)' },
  calcSubsidy: { uz: 'Davlat subsidiyasi', ru: 'Господдержка (субсидия)' },
  calcNetCost: { uz: 'Sizning xarajatingiz', ru: 'Ваши затраты' },
  calcAnnualSaving: { uz: 'Yillik tejamkorlik', ru: 'Годовая экономия' },
  calcPayback: { uz: 'O‘zini oqlash muddati', ru: 'Срок окупаемости' },
  calcYears: { uz: 'yil', ru: 'лет' },
  calcMonths: { uz: 'oy', ru: 'мес' },
  calcForField: { uz: '{n} ga dalangiz uchun', ru: 'для вашего поля {n} га' },
  calcAlreadyDrip: { uz: 'Siz allaqachon tomchilatib sug‘oryapsiz — eng tejamkor usul!', ru: 'Вы уже на капельном поливе — самый экономный способ!' },
  calcNote: { uz: 'Hisob suv tejamkorligiga asoslangan; hosil ortishi hisobga olinmagan.', ru: 'Расчёт по экономии воды; прибавка урожая не учтена.' },
  mln: { uz: 'mln', ru: 'млн' },
  // Crop market prices
  ...
  priceIndicative: { uz: 'Taxminiy narxlar ({m}). Aniq narx bozorga bog‘liq.', ru: 'Ориентировочные цены ({m}). Точная зависит от рынка.' },
