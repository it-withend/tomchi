...
// fields, plus a per-region breakdown for the map and the regions ranking.
// Uses the Supabase service role (bypasses RLS) and computes savings with the
// same FAO-56 engine as the app. No personal data is returned — only anonymous
// aggregates.
...
export default async (): Promise<Response> => {
  ...
  try {
    ...
    const payload = {
      ...
      // Count only regions that actually save water, to match the ranking below.
      regionCount: Object.values(byRegion).filter((r) => r.saved > 0).length,
